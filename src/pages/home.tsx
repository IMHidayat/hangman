...
function Home() {
  ...
  return (
    <>
      <main className="flex py-4 gap-2 items-center flex-col">
        <div className={` ${winOrLose === "" ? "hidden" : "grid"} my-4 gap-2`}>
        ...
        <article className=" w-full">
          <div className="mb-4 mx-auto w-fit relative">
        ...
        {/* WORD TO GUESS */}
        <article className="flex px-4 mb-8 gap-2 md:gap-10 w-full justify-center  uppercase font-bold">
          {wordToGuess.map((v, i) => {
            return (
              <span
                className={`flex-1 pb-2 w-6  md:text-8xl text-center border-b-black border-b-8 ${
        ...
        {/* KEYBOARD */}
        <article className="flex gap-1 w-full flex-col items-center border border-black">
          {["qwertyuiop", "asdfghjkl", "zxcvbnm"].map((word, i) => {
            return (
              <div key={i} className="flex gap-1 ">
                {word.split("").map((letter, i2) => {
                  return (
                    <button
                      className={`px-3 py-1.5 font-semibold text-gray-800 rounded-lg border border-gray-500 
                      ${
